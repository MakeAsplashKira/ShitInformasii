...
public class VigenerCipherWindow extends JFrame{
    ...
    private JPanel createHeaderPanel() {
        ...
        JLabel titleLabel = new JLabel("Шифр Виженера");
    ...
    private JPanel createControlPanel() {
        ...
        JLabel keyLabel = new JLabel("Ключ (слово):");
        keyLabel.setForeground(TEXT_PRIMARY);
        keyLabel.setFont(new Font("Segoe UI", Font.BOLD, 14));


        JTextField keyTextField = new JTextField(7);
        keyTextField.setBackground(INPUT_BACKGROUND);
        keyTextField.setForeground(TEXT_PRIMARY);
        keyTextField.setCaretColor(TEXT_PRIMARY);
        keyTextField.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        keyTextField.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(new Color(60, 60, 60), 1),
                BorderFactory.createEmptyBorder(5, 5, 5, 5)
        ));
        keyTextField.setText("ключ");
    ...
    private void processText() {
        ...
        String result = encrypt? VigenerCipher.encrypt(input, keyword):
                                 VigenerCipher.decrypt(input, keyword);
        outputTextArea.setText(result);
        if(encrypt) {
            CipherFileManager.saveEncryptedMessage(this, result, keyword, "Vigener");
        }
    }
}
